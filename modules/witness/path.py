...
class Path:
    ...
    def draw(self):
        ...
        for i in range(len(self.steps) - 1):
            for p in range(self.puzzle.cell_size):
                ...
                position = Point(self.puzzle.offset.x + self.steps[i].x * self.puzzle.cell_size + (
                self.steps[i + 1].x - self.steps[i].x) * p,
                                 self.puzzle.offset.y + self.steps[i].y * self.puzzle.cell_size + (
                                 self.steps[i + 1].y - self.steps[i].y) * p)
        ...
        position = Point(self.puzzle.offset.x + self.steps[len(self.steps) - 1].x * self.puzzle.cell_size,
                         self.puzzle.offset.y + self.steps[len(self.steps) - 1].y * self.puzzle.cell_size)
        color = blend_colors(self.screen.pixel[position.x][position.y], path_color,
                             1.0 - min(1, max(0, offset * self.puzzle.cell_size)))
        if opacity != 1:
            color = blend_colors(self.screen.pixel[position.x][position.y], color, opacity)
        self.screen.pixel[position.x][position.y] = color
        ...
        if self.steps[len(self.steps) - 1] == self.puzzle.exit and offset == 0:
            ...
            if opacity != 1:
                color = blend_colors(self.screen.pixel[self.puzzle.exit_on_screen.x][self.puzzle.exit_on_screen.y],
                                     color, opacity)
        ...
        if len(self.steps) > 0:
            for x in range(3):
                for y in range(3):
                    ...
                    pos = Point(self.puzzle.offset.x + x + self.steps[0].x * self.puzzle.cell_size - 1,
                                self.puzzle.offset.y + y + self.steps[0].y * self.puzzle.cell_size - 1)
        ...
        if self.wrong:
            for error in self.errors:
                self.screen.pixel[error.x][error.y] = blend_colors(self.screen.pixel[error.x][error.y],
                                                                   Color(255, 0, 0), math.sin(time.clock() * 8) ** 2)
    ...
    def contains_edge(self, point1, point2):
        for i in range(len(self.steps) - 1):
            if (self.steps[i] == point1 and self.steps[i + 1] == point2) or (
                    self.steps[i] == point2 and self.steps[i + 1] == point1):
                return True
        return False
    ...
    def extend(self):
        ...
        nodes = [Point(block.x, block.y), Point(block.x + 1, block.y), Point(block.x + 1, block.y + 1),
                 Point(block.x, block.y + 1)]
    ...
    def fill_zone(self, x, y, value):
        ...
        while len(queue) != 0:
            ...
            if block.x > 0 and self.zones[block.x - 1][block.y] == -1 and not self.contains_edge(block, Point(block.x,
                                                                                                              block.y + 1)):
                self.zones[block.x - 1][block.y] = value
                queue.append(Point(block.x - 1, block.y))
            if block.y > 0 and self.zones[block.x][block.y - 1] == -1 and not self.contains_edge(block,
                                                                                                 Point(block.x + 1,
                                                                                                       block.y)):
                self.zones[block.x][block.y - 1] = value
                queue.append(Point(block.x, block.y - 1))
            if block.x < self.puzzle.width - 2 and self.zones[block.x + 1][block.y] == -1 and not self.contains_edge(
                    Point(block.x + 1, block.y), Point(block.x + 1, block.y + 1)):
                self.zones[block.x + 1][block.y] = value
                queue.append(Point(block.x + 1, block.y))
            if block.y < self.puzzle.height - 2 and self.zones[block.x][block.y + 1] == -1 and not self.contains_edge(
                    Point(block.x, block.y + 1), Point(block.x + 1, block.y + 1)):
                self.zones[block.x][block.y + 1] = value
                queue.append(Point(block.x, block.y + 1))
