...
class Snake(Module):
    ...
    def new_game(self):
        ...
        while time.clock() < start + t:
            self.screen.clear()
            self.screen.pixel[self.snake[0].x][self.snake[0].y] = Color(
                int(self.head_color.r * (time.clock() - start) ** 2 / t),
                int(self.head_color.g * (time.clock() - start) ** 2 / t),
                int(self.head_color.b * (time.clock() - start) ** 2 / t))
            self.screen.update()
    ...
    def game_over(self):
        ...
        while time.clock() < start + t:
            ...
            self.screen.clear()
            self.screen.pixel[self.snake[0].x][self.snake[0].y] = Color(
                int(self.head_color.r * (1 - (time.clock() - start) / t) ** 2),
                int(self.head_color.g * (1 - (time.clock() - start) / t) ** 2),
                int(self.head_color.b * (1 - (time.clock() - start) / t) ** 2))
            self.screen.update()
                    ...
                    for y in range(self.screen.height):
                        ...
                        if (d / radius) ** 0.5 < (time.clock() - self.pulse_offset) / t:
                            self.screen.pixel[x][y] = Color(
                                max(0,
                                    int(self.food_color.r * 0.2 * (1 - (time.clock() - self.pulse_offset) / t) ** 2)),
                                max(0,
                                    int(self.food_color.g * 0.2 * (1 - (time.clock() - self.pulse_offset) / t) ** 2)),
                                max(0,
                                    int(self.food_color.b * 0.2 * (1 - (time.clock() - self.pulse_offset) / t) ** 2)))
