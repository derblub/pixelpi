...
def map(value, old_lower, old_upper, new_lower, new_upper):
    return max(new_lower,
               min(new_upper, new_lower + (new_upper - new_lower) * (value - old_lower) / (old_upper - old_lower)))
...
class Fire(Module):
    ...
    def get_fire_value(self, x, y, t):
        ...
        value = map(snoise2(x * 3, stretch ** (-1.0 / exp) * (1.0 - ((1.0 - y) * stretch) ** exp) * 0.8 - t * 0.7, 4),
                    -0.72, 0.7, 0, 1)
        value = value ** 1.5
